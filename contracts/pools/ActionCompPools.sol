...
contract ActionCompPools is OwnableUpgradeable, IActionPools, IClaimFromBank {
    ...
    // mint from yextoken, when reward token is YEXToken GRAPToken, mint it
    mapping(address => uint256) public mintTokens;
    // mint for dev, while mint token, mint a part for dev
    address public dev;
    ...
    function initialize(address _bank, address[] memory _mintTokens, uint256[] memory _mintFee, address _dev) public initializer {
        __Ownable_init();
        bank = _bank;
        dev = _dev;
    ...
    function setDev(address _dev) external {
        require(msg.sender == dev, 'prev dev only');
        dev = _dev;
    }
        ...
        if (poolReward > 0) {
            ...
            if( mintTokens[rewardToken] > 0) {
                ...
                if(mintTokens[rewardToken] > 1e18) {
                    ...
                    IRewardsToken(rewardToken).mint(dev, mintFee);   // mint for dev
                }
