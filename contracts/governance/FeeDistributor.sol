...
contract FeeDistributor is OwnableUpgradeable {
    ...
    uint256 private _releaseRate;  /// rate for 1e9
    ...
    uint256 public SETTLEMENT_TIME_RATE;
    uint256 public periodUnit;
    ...
    function initialize(
        ...
        address admin_,
        uint256 adminFeeRate_) external initializer {
        __Ownable_init();
        ...
        _releaseRate = IVotingEscrow(votingEscrow_).releaseRate();
        SETTLEMENT_TIME_RATE = IVotingEscrow(votingEscrow_).SETTLEMENT_TIME_RATE();
        periodUnit = IVotingEscrow(votingEscrow_).periodUnit();
        admin = admin_;
    ...
    function syncWithVotingEscrow(address account) external {
        ...
        uint256 nextWeek = _endOfPeriod(block.timestamp);
        ...
        if (oldLockedBalance.amount > 0 && oldLockedBalance.unlockTime > nextWeek) {
            scheduledUnlock[oldLockedBalance.unlockTime] = 
                scheduledUnlock[oldLockedBalance.unlockTime].sub(oldLockedBalance.amount);
            newNextWeekLocked = newNextWeekLocked.sub(oldLockedBalance.amount);
            newNextWeekSupply = newNextWeekSupply.sub(
                oldLockedBalance.amount.mul(oldLockedBalance.unlockTime.sub(nextWeek))
                        .div(_maxTime).mul(_releaseRate).div(1e9)
            );
        }
        ...
        nextWeekSupply = newNextWeekSupply.add(
            newLockedBalance.amount.mul(newLockedBalance.unlockTime.sub(nextWeek))
                .add(_maxTime.sub(1)).div(_maxTime).mul(_releaseRate).div(1e9)
        );
    ...
    function checkpoint() public {
        ...
        uint256 weekCursor = _endOfPeriod(rewardTime).sub(periodUnit);
        uint256 currentWeek = _endOfPeriod(block.timestamp).sub(periodUnit);
        ...
        if (weekCursor < currentWeek) {
            ...
            for (uint256 w = weekCursor.add(periodUnit); w <= currentWeek; w += periodUnit) {
                veSupplyPerWeek[w] = newSupply;
                // Calculate supply at the end of the next week.
                newSupply = newSupply.sub(newLocked.mul(periodUnit).div(_maxTime).mul(_releaseRate).div(1e9));
                // Remove Chess unlocked at the end of the next week from total locked amount.
                newLocked = newLocked.sub(scheduledUnlock[w.add(periodUnit)]);
            }
        ...
        if (tokensToDistribute > 0) {
            if (weekCursor >= currentWeek) {
                ...
                rewardsPerWeek[weekCursor] = rewardsPerWeek[weekCursor].add(
                    tokensToDistribute.mul(weekCursor + periodUnit - rewardTime) / sinceLast
                );
                weekCursor += periodUnit;
                // Calculate the fraction of rewards for intermediate whole weeks.
                while (weekCursor < currentWeek) {
                    rewardsPerWeek[weekCursor] = tokensToDistribute.mul(periodUnit) / sinceLast;
                    weekCursor += periodUnit;
                }
                // Calculate the fraction of rewards proportional to the time from
    ...
    function calibrateSupply() external {
        uint256 nextWeek = _endOfPeriod(checkpointTimestamp);
